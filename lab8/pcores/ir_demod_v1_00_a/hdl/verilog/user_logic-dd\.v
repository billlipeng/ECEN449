...
//----------------------------------------------------------------------------
  ...
  parameter start_cycle_num_max = 190000;//177000
  ...
  begin
	...
	if(!IR_signal)
		cnt  <=  cnt + 1;
	if( IR_signal == 1 && cnt > start_cycle_num_min && cnt < start_cycle_num_max) // start
	begin
		start_flag <= 1;
		data_bit <= 0;
		data <= 0;
	...
	else if( IR_signal == 1 && (cnt < zero_cycle_num_min || cnt > start_cycle_num_max)) // error
	begin // error here 
		start_flag <= 0;
